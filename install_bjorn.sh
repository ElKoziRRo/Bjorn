...
setup_bjorn() {
    ...
    cd /home/$BJORN_USER
    if [ -d "Bjorn" ]; then
        log "INFO" "Using existing CyberYanush directory"
        echo -e "${GREEN}Using existing CyberYanush directory${NC}"
    else
        # No existing directory, proceed with clone
        log "INFO" "Cloning CyberYanush repository"
        git clone https://github.com/ElKoziRRo/Bjorn.git
        check_success "Cloned CyberYanush repository"
    fi
